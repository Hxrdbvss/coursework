...
import { Link, useNavigate } from 'react-router-dom'; // Оставляем useNavigate
...
function SurveyList({ token, user, handleLogout }) {
  ...
  const navigate = useNavigate(); // Используем для редиректа после logout
  ...
  const onLogout = () => {
    handleLogout();
    navigate('/register'); // Используем navigate
  };

  return (
    <>
      <h1>Список опросов</h1>
      <Button variant="danger" onClick={onLogout} className="mb-3 btn-custom">
        Выйти
      </Button>
      <ListGroup>
        {surveys.map(survey => (
